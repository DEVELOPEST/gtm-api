...
use crate::domain::timeline::mapper::{map_activity, map_subdir_level_timeline, map_timeline};
use crate::domain::timeline::resources::{ActivityJson, IntervalJson, SubdirLevelTimelineJsonEntry, SubdirLevelTimelineJsonWrapper, SubdirLevelTimelineJson};
...
pub fn get_subdir_level_timeline(
    ...
    let mut users: Vec<&String> = file_edits_data.iter()
        .map(|e| &e.user)
        .collect();
    users.sort();
    users.dedup();
    ...
    let data: Vec<SubdirLevelTimelineJson> =
        ...
        .into_iter()
            ...
            .collect();
    let mut paths: Vec<String> = data.iter()
        .map(|e| e.directories.iter()
            .map(|(k, _)| k.clone())
            .collect::<Vec<String>>())
        .flatten()
        .collect::<Vec<String>>();
